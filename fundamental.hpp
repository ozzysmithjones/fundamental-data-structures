...
#if defined(NDEBUG)
#define ST_BREAKPOINT() ((void)0)
#elif defined(__GNUC__)
#define ST_BREAKPOINT() __builtin_trap()
#elif defined(_MSC_VER)
#define ST_BREAKPOINT() __debugbreak()
#else
#define ST_BREAKPOINT() ((void)0)
#endif
...
#ifndef NDEBUG
#define ST_BUG(...) std::print(__FILE__":" ST_TOSTRING(__LINE__) " \"{}\" ST_BUG found.\n", __func__); std::print("" __VA_ARGS__); std::fflush(stdout); ST_BREAKPOINT()
#else
#define ST_BUG(...) ((void)0)
#endif

#define ST_ASSERT(condition, fallback, ...) \
...
#ifndef NDEBUG
#define ST_DEBUG_ASSERT(condition, fallback, ...) ST_ASSERT(condition, fallback, __VA_ARGS__)
#else
#define ST_DEBUG_ASSERT(condition, fallback, ...) ((void)0)
    ...
    struct heap_allocator {
        static inline void* malloc(size_t alignment, size_t bytes) {
            ST_ASSERT(alignment <= alignof(std::max_align_t), return nullptr, "Requested alignment {} exceeds max supported alignment {} for default heap allocator.", alignment, alignof(std::max_align_t));
        ...
        static inline void free(void* ptr, size_t alignment, size_t bytes) {
            ST_ASSERT(alignment <= alignof(std::max_align_t), return, "Requested alignment {} exceeds max supported alignment {} for default heap allocator.", alignment, alignof(std::max_align_t));
            std::free(ptr);
        }
    };
    ...
    struct block_allocator {
        ...
        void* malloc(size_t alignment, size_t bytes) {
            ST_ASSERT(last_block != nullptr, return nullptr, "Out of memory in block_allocator.");
            ST_ASSERT(alignment <= Alignment, return nullptr, "Requested alignment {} exceeds block allocator alignment {}", alignment, Alignment);
            ST_ASSERT(bytes <= Block_Size, return nullptr, "Requested size {} exceeds block size {}", bytes, Block_Size);
            block* b = last_block;
        ...
        void* realloc(void* ptr, size_t alignment, size_t old_bytes, size_t new_bytes) {
            ...
            ST_BUG("Block allocator cannot grow allocations beyond block size.");
            return nullptr;
        }
        ...
        void free(void* ptr, size_t alignment, size_t bytes) {
            ...
            ST_ASSERT(alignment <= Alignment, , "Requested alignment {} exceeds block allocator alignment {}", alignment, Alignment);
            ST_ASSERT(bytes <= Block_Size, , "Requested size {} exceeds block size {}", bytes, Block_Size);
        }
    ...
    public:
        ...
        constexpr enum_array(std::initializer_list<T> init)  requires (std::is_default_constructible_v<T>) : elements{} {
            ST_ASSERT(init.size() <= Capacity, return, "Initializer list size exceeds enum_array capacity.");
            size_t i = 0;
        ...
        constexpr result attempt_copy_element(Enum e, T& out_value) const {
            ...
            if (index >= Capacity) [[unlikely]] {
                ST_BUG("Index out of bounds (index={}, limit={})", index, Capacity);
    ...
    public:
        ...
        constexpr fixed_array(std::initializer_list<T> init) {
            ST_ASSERT(init.size() <= Capacity, return, "Initializer list size exceeds fixed_array capacity.");
            size_t i = 0;
            for (const T& value : init) {
    ...
    public:
        ...
        explicit fixed_heap_array(size_type capacity) requires (std::is_default_constructible_v<T>) : allocator(), elements(nullptr), capacity(0) {
            ...
            ST_ASSERT(elements != nullptr, return, "Out of memory, cannot allocate fixed_heap_array.");
        ...
        fixed_heap_array(std::initializer_list<T> init) : allocator(), elements(nullptr), capacity(0) {
            if (init.size() == 0) {
                return;
            }
        ...
        fixed_heap_array& operator=(const fixed_heap_array& other) {
            ...
            elements = reinterpret_cast<pointer>(allocator.malloc(alignof(T), sizeof(T) * other.capacity));
            ST_ASSERT(elements != nullptr, return *this, "Out of memory, cannot allocate fixed_heap_array.");
            this->capacity = other.capacity;
            for (size_type i = 0; i < other.capacity; ++i) {
                new (&elements[i]) T(other.elements[i]);
            }
    ...
    public:
        ...
        result append_multiple(const_pointer values, size_type num_values) {
            pointer elements = reinterpret_cast<pointer>(elements_data);
            ST_ASSERT(values != nullptr || num_values == 0, return result::failure, "values cannot be null if num_values is greater than 0.");
        ...
        result remove(size_type index) {
            ...
            pointer elements = reinterpret_cast<pointer>(elements_data);
            ST_ASSERT(index < count, return result::failure, "Index out of bounds (index={}, limit={})", index, count);
        ...
        result attempt_copy_element(size_type index, T& out_value) const {
            ...
            if (index >= count) [[unlikely]] {
                ST_BUG("Index out of bounds (index={}, limit={})", index, count);
                return result::failure;
            }
            out_value = elements[index];
            return result::success;
    ...
    public:
        ...
        void set_allocator(const Allocator& new_allocator) {
            ST_ASSERT(count == 0, return, "Cannot change allocator of dynamic_array when it is not empty.");
            if (elements != nullptr) {
                allocator.free(elements, alignof(T), sizeof(T) * capacity);
                elements = nullptr;
                count = 0;
        ...
        result append(Arg&& value) requires (std::is_same_v<std::remove_cvref_t<Arg>, T>) {
            if (count == capacity) {
                ...
                ST_ASSERT(new_elements != nullptr, return result::failure, "Out of memory, cannot append element to dynamic_array.");
        ...
        result append_multiple(const_pointer values, size_type num_values) {
            ST_ASSERT(values != nullptr || num_values == 0, return result::failure, "values cannot be null if num_values is greater than 0 in dynamic_array::append_multiple.");
            while (count + num_values > capacity) {
                ...
                ST_ASSERT(new_elements != nullptr, return result::failure, "Out of memory, cannot append multiple elements to dynamic_array.");
                elements = new_elements;
                capacity = new_capacity;
            }
            for (size_type i = 0; i < num_values; ++i, ++count) {
        ...
        void preallocate_memory_for(size_type additional_expected_elements) {
            ...
            ST_ASSERT(new_elements != nullptr, return, "Out of memory, cannot preallocate space in dynamic_array.");
            elements = new_elements;
            capacity = new_capacity;
        }
    ...
    public:
        ...
        constexpr enum_bitset(std::initializer_list<T> init) : bits(0) {
            for (const T& value : init) {
                ...
                ST_ASSERT(index < bit_count, continue, "Index out of bounds in enum_bitset initializer list.");
        ...
        constexpr bool contains(T value) const {
            ...
            if (index >= bit_count) [[unlikely]] {
                ST_BUG("Index out of bounds (index={}, limit={})", index, bit_count);
                return false;
            }
            if constexpr (Kind == enum_value::indices) {
        ...
        constexpr void insert(T value) {
            integer index = static_cast<integer>(value);
            ST_ASSERT(index < bit_count, return, "Index out of bounds (index={}, limit={})", index, bit_count);
            if constexpr (Kind == enum_value::indices) {
                bits |= (static_cast<integer>(1) << index);
            }
            else {
    ...
    public:
        ...
        explicit heap_pointer(const T& value) requires std::is_copy_constructible_v<T> : allocator(), ptr(nullptr) {
            ...
            ST_ASSERT(ptr != nullptr, return, "Out of memory, cannot allocate heap_pointer.");
            new (ptr) T(value);
        }
        ...
        result default_construct() requires std::is_default_constructible_v<T> {
            if (ptr == nullptr) {
                ptr = reinterpret_cast<pointer>(allocator.malloc(alignof(T), sizeof(T)));
                ST_ASSERT(ptr != nullptr, return result::failure, "Out of memory, cannot allocate heap_pointer.");
                new (ptr) T();
            }
            else {
                *ptr = T();
        ...
        reference value_or(reference& fallback) {
            if (ptr == nullptr) [[unlikely]] {
                ST_BUG("Dereferencing null heap_pointer.");
                return fallback;
            }
            return *ptr;
        }
    ...
    public:
        ...
        result attempt_copy_value(T& out_value) const {
            if (!has_value) [[unlikely]] {
                ST_BUG("Maybe type does not contain a value.");
                return result::failure;
            }
