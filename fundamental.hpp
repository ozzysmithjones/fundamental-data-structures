...
// Warnings can be useful to notify when something is being done inefficiently (but not incorrectly), or if something is deprecated.
#ifndef NDEBUG
#define WARNING(...) std::print(__FILE__":" TOSTRING(__LINE__) " \"{}\" warning.\n", __func__); std::print("" __VA_ARGS__); std::fflush(stdout)
#else
...
public:
    virtual void* malloc(size_t alignment, size_t bytes) = 0;
    virtual void* realloc(void* ptr, size_t alignment, size_t old_bytes, size_t new_bytes) = 0;
    virtual void free(void* ptr, size_t alignment, size_t bytes) = 0;
    virtual ~allocator_base() = default;
};

class heap_allocator final : public allocator_base {
public:
    void* malloc(size_t alignment, size_t bytes) override;
    void* realloc(void* ptr, size_t alignment, size_t old_bytes, size_t new_bytes) override;
    void free(void* ptr, size_t alignment, size_t bytes) override;
};

class debug_allocator final : public allocator_base {
public:
    debug_allocator(allocator_base& wrapped_allocator);
    ~debug_allocator() override;
    void* malloc(size_t alignment, size_t bytes) override;
    void* realloc(void* ptr, size_t alignment, size_t old_bytes, size_t new_bytes) override;
    void free(void* ptr, size_t alignment, size_t bytes) override;

private:
    allocator_base& wrapped_allocator;
    size_t total_allocation_count = 0;
    size_t current_allocation_count = 0;
...
public:
    ...
    fixed_heap_array(allocator_base& allocator = global_heap_allocator) : allocator(allocator), elements(nullptr), capacity(0) {
    }

    fixed_heap_array(size_t capacity, allocator_base& allocator = global_heap_allocator) requires (std::is_default_constructible_v<T>) : allocator(allocator), elements(nullptr), capacity(0) {
        if (capacity == 0) {
            return;
        }
    ...
    fixed_heap_array(std::initializer_list<T> init, allocator_base& allocator = global_heap_allocator) requires (std::is_copy_constructible_v<T>) : allocator(allocator), elements(nullptr), capacity(0) {
        if (init.size() == 0) {
            return;
        }

        size_type capacity = init.size();
        elements = reinterpret_cast<pointer>(allocator.malloc(alignof(T), sizeof(T) * capacity));
        ASSERT(elements != nullptr, return, "Out of memory, cannot allocate fixed_heap_array.");
        this->capacity = capacity;
    ...
    fixed_heap_array(const fixed_heap_array& other, allocator_base& allocator = global_heap_allocator) : allocator(allocator), elements(nullptr), capacity(0) {
        if (other.capacity == 0) {
            return;
        }
    ...
    fixed_heap_array(fixed_heap_array&& other, allocator_base& allocator = global_heap_allocator) noexcept : allocator(allocator), elements(other.elements), capacity(other.capacity) {
    ...
    result construct_or_set_default_values(size_t capacity) requires (std::is_default_constructible_v<T>) {
        if (capacity == 0) {
            if (this->capacity != 0) {
                if constexpr (!std::is_trivially_destructible_v<T>) {
                ...
                allocator.free(elements, alignof(T), sizeof(T) * this->capacity);
                elements = nullptr;
            }
            return result::success;
        }
        if (elements != nullptr) {
            if (this->capacity != 0 && !std::is_trivially_destructible_v<T>) {
                for (size_type i = 0; i < this->capacity; ++i) {
                    std::destroy_at(&elements[i]);
                }
            }
        ...
        ASSERT(elements != nullptr, return result::failure, "Out of memory, cannot allocate fixed_heap_array.");
        this->capacity = capacity;
        for (size_type i = 0; i < capacity; ++i) {
            new (&elements[i]) T();
        }
    ...
    result construct_or_set_values(std::initializer_list<T> init) requires (std::is_copy_constructible_v<T>) {
        if (init.size() == 0) {
            if (this->capacity != 0) {
                if constexpr (!std::is_trivially_destructible_v<T>) {
                    for (size_type i = 0; i < this->capacity; ++i) {
        ...
        if (elements != nullptr) {
            if (capacity != 0 && !std::is_trivially_destructible_v<T>) {
                for (size_type i = 0; i < capacity; ++i) {
                    std::destroy_at(&elements[i]);
                }
            }
    ...
    void set_allocator(allocator_base& new_allocator) {
        if (elements != nullptr) {
            allocator.free(elements, alignof(T), sizeof(T) * capacity);
            elements = nullptr;
        }
...
public:
    ...
    capped_array(std::initializer_list<T> init) : elements_data{}, count(0) {
        ASSERT(init.size() <= Capacity, return, "Initializer list size exceeds capped_array capacity.");
        for (const T& value : init) {
            new (&reinterpret_cast<pointer>(elements_data)[count]) T(value);
            ++count;
        }
    }
    ...
    result append_multiple(std::initializer_list<T> values) {
        return append_multiple(values.begin(), values.size());
    }
...
public:
    ...
    dynamic_array(size_t capacity, allocator_base& allocator = global_heap_allocator) : allocator(allocator), elements(nullptr), count(0), capacity(0) {
        ...
        elements = reinterpret_cast<pointer>(allocator.malloc(alignof(T), sizeof(T) * capacity));
        ASSERT(elements != nullptr, return, "Out of memory, cannot allocate dynamic_array.");
        this->capacity = capacity;
    }

    dynamic_array(std::initializer_list<T> init, allocator_base& allocator = global_heap_allocator) : allocator(allocator), elements(nullptr), count(0), capacity(0) {
        if (init.size() == 0) {
            return;
        }
        append_multiple(init.begin(), init.size());
    }

    dynamic_array(const dynamic_array& other, allocator_base& allocator = global_heap_allocator) : allocator(allocator), elements(nullptr), count(0), capacity(0) {
        append_multiple(other.elements, other.count);
    }
    dynamic_array(dynamic_array&& other, allocator_base& allocator = global_heap_allocator) noexcept : allocator(allocator), elements(other.elements), count(other.count), capacity(other.capacity) {
        elements = other.elements;
        count = other.count;
        capacity = other.capacity;
        other.elements = nullptr;
...
public:
    ...
    explicit heap_pointer(const T& value, allocator_base& allocator = global_heap_allocator) requires std::is_copy_constructible_v<T> : allocator(allocator), ptr(nullptr) {
    ...
    explicit heap_pointer(T&& value, allocator_base& allocator = global_heap_allocator) requires std::is_move_constructible_v<T> : allocator(allocator), ptr(nullptr) {
        ptr = reinterpret_cast<pointer>(allocator.malloc(alignof(T), sizeof(T)));
        ASSERT(ptr != nullptr, return, "Out of memory, cannot allocate heap_pointer.");
        new (ptr) T(std::move(value));
    }

    heap_pointer(const heap_pointer& other, allocator_base& allocator = global_heap_allocator) requires std::is_copy_constructible_v<T> : allocator(allocator), ptr(nullptr) {
    ...
    heap_pointer(heap_pointer&& other, allocator_base& allocator = global_heap_allocator) noexcept : allocator(allocator), ptr(other.ptr) {
        other.ptr = nullptr;
    }
    ...
    bool heap_value_equals(const T& other) const {
        if (ptr == nullptr) {
            return false;
        }
        return *ptr == other;
    ...
    reference heap_value_or(reference& fallback) {
    ...
    const_reference heap_value_or(const_reference fallback) const {
        if (ptr == nullptr) [[unlikely]] {
            BUG("Dereferencing null heap_pointer.");
            return fallback;
        }
    ...
    reference heap_value_unsafe() {
        DEBUG_ASSERT(ptr != nullptr, static T fallback{}; return fallback, "Dereferencing null heap_pointer.");
        return *ptr;
    }

    const_reference heap_value_unsafe() const {
        DEBUG_ASSERT(ptr != nullptr, static T fallback{}; return fallback, "Dereferencing null heap_pointer.");
        return *ptr;
    }
...
public:
    ...
    [[nodiscard]] T& value_unsafe() {
        DEBUG_ASSERT(has_value, return *reinterpret_cast<T*>(&value_data), "Maybe type does not contain a value.");
        return *reinterpret_cast<T*>(&value_data);
    }
